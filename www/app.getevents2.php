...
if ($requestkey == 1) {
  //This is the current public key, length 256
  //{"e":"10001","n":"9e4ffbcb886d88b2f5b2cd4a57534ae201d940a2a25a1ebe74d7603e9a4ad3af","maxdigits":"35"}
  echo "10001\n" .
    "9e4ffbcb886d88b2f5b2cd4a57534ae201d940a2a25a1ebe74d7603e9a4ad3af\n" .
    "35";
  die();
}

///////jcryption.php begin
// TODO: Most likely there's standard php function for that. Or at least a library available through Composer
/**
 * jCryption
 *
...
$dbpassHash = $stm->executeScalar();

//grab the password, decrypt it, then custom_hash it.

//This is the current private key, length 256.
//{"d":"87c02434ca3eca9a528f48f3f75fa7f4381fdc4c39b0073f96fffe9722170149","n":"9e4ffbcb886d88b2f5b2cd4a57534ae201d940a2a25a1ebe74d7603e9a4ad3af","maxdigits":"35"}
//d = 61401719081263037181678367815398937621951419584827093870116271679392185778505
//n = 71606748819843330921146238972147982542847477646731462461647964280966643110831
$jCryption = new jCryption;
$pass = $jCryption->decrypt($pass, "61401719081263037181678367815398937621951419584827093870116271679392185778505", "71606748819843330921146238972147982542847477646731462461647964280966643110831");

if (!SecurityUtil::verifyPassword($pass, $dbpassHash)) {
  die ("BAD LOGIN");
}

echo "OK LIST\n";

$stm = $db->prepare("SELECT ch.name FROM chars ch INNER JOIN newevents ne ON ne.person = ch.ID AND ne.new = 0
  WHERE ch.status = 1 AND ch.player = :playerId");
$stm->bindInt("playerId", $userid);
$stm->execute();
